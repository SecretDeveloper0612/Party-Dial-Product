...
function CheckoutContent() {
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      if (result.status === "success") {
        // Merge database plans with professional defaults
  ...
  useEffect(() => {
    if (prePlanId && plans.length > 0) {
      const p = plans.find(pl => pl.$id === prePlanId || pl.name.toLowerCase().includes(prePlanId.toLowerCase()));
      if (p) {
        setSelectedPlan(p);
        setStep(preVenueId ? 3 : 2); // Skip directly to Review if venue is pre-identified
      }
    }
  }, [prePlanId, plans, preVenueId]);
  ...
  const calculateTotal = () => {
    ...
    let addonPrice = 0;
    if (preAddonId) {
       if (preAddonId === 'a50') addonPrice = 1999;
       if (preAddonId === 'a100') addonPrice = 2999;
       if (preAddonId === 'a200') addonPrice = 3999;
       if (preAddonId === 'a500') addonPrice = 6999;
       if (preAddonId === 'a1000') addonPrice = 9999;
       if (preAddonId === 'a2000') addonPrice = 14999;
    }

    const subtotal = selectedPlan.price + addonPrice;
    let discount = 0;

    if (preDiscount) {
       discount = (subtotal * parseFloat(preDiscount)) / 100;
    }
          ...
          {step === 1 && (
            ...
            >
              <div className="text-center">
                 <h2 className="text-3xl font-black text-slate-800 tracking-tight">Activate Membership</h2>
                 <p className="text-slate-400 font-medium mt-2">Securely process your premium subscription payment</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {plans
                  .filter(p => !prePlanId || selectedPlan?.$id === p.$id)
                  .map((p) => (
                    <button 
                      key={p.$id}
                      onClick={() => setSelectedPlan(p)}
                      className={cn(
                        "p-8 bg-white border-2 rounded-[2rem] text-left transition-all relative overflow-hidden group col-span-full",
            ...
            >
              <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/50 border border-slate-50 overflow-hidden">
                 ...
                 <div className="p-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          <div className="flex items-center justify-between">
                             <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Venue Owner Name</label>
                             {isLocked && (
                                <button onClick={() => setIsLocked(false)} className="text-[#b66dff] hover:opacity-70 transition-opacity">
                                   <Pencil size={12} />
                                </button>
                             )}
                          </div>
                    ...
                    <div className="space-y-2">
                       <div className="flex items-center justify-between">
                          <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Billing Address Details</label>
                          {isLocked && (
                             <button onClick={() => setIsLocked(false)} className="text-[#b66dff] hover:opacity-70 transition-opacity">
                                <Pencil size={12} />
                             </button>
                       ...
                       <textarea 
                          ...
                          placeholder={isLocked ? "Street address not provided in profile" : "Complete street address"}
          ...
          {step === 3 && (
            ...
            >
              <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 mb-8">
                 ...
                 <div className="p-8 space-y-6">
                    {/* Partner Review Section for Pre-identified Venues */}
                    ...
                    <div className="flex items-center justify-between px-2">
                       <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Growth Tier Membership</p>
                       </div>
                       <span className="text-sm font-black text-slate-800">₹{totals.base.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="space-y-4 pt-6 border-t-2 border-dashed border-slate-100">
                       ...
                       {totals.discount > 0 && (
                         <div className="flex items-center justify-between text-xs font-black text-emerald-500 uppercase tracking-widest">
                    ...
                    {!isLocked && (
                        <button 
                           onClick={() => setStep(2)}
                           className="w-full text-center text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-slate-500 transition-colors py-2"
                        >
              ...
              <div className="flex items-center justify-center gap-4 text-slate-400 bg-white/50 py-4 rounded-3xl border border-white">
                 <ShieldCheck size={20} className="text-emerald-500" />
                 <p className="text-[10px] font-black uppercase tracking-widest opacity-80">Secure 256-bit Encrypted Checkout</p>
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl mb-8">
                 ...
                 <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-black text-slate-700">#PAY-{Date.now().toString().slice(-6)}</span>
                 </div>
                 <div className="flex items-center justify-between">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 opacity-60 mb-12">
